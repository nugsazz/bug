# -*- coding: utf-8 -*-
	...
	importLINETCR
	fromLINETCR.lib.curve.ttypes import*
	...
	helpMessage =""" Zeus Bot >> red check mark >>
	
	􀔃􀅕red arrow right>> Command Public
	[Bot]     Cek Akun Bot
	...
	􀔃􀅕red arrow right >> Command Private
...
SSH BY Jim line.me/ti/p/
	"""
	

	Setgroup =""" Privasi Menu Zeus Bot􀔃􀄆red check mark>>
...
SSH BY Jim line.me/ti/p/
	...
	defsendMessage(to, text, contentMetadata={}, contentType=0):
	    mes = Message()
	    mes.to, mes.from_ = to, profile.mid
	    mes.text = text
	    mes.contentType, mes.contentMetadata = contentType, contentMetadata
	if to notin messageReq:
	        messageReq[to] =-1
	    messageReq[to] +=1
	

	defNOTIFIED_READ_MESSAGE(op):
	try:
	...
	iflen(G.members) <= wait["autoCancel"]["members"]:
	...
	if wait["lang"] =="JP":
	                        ...
	                        cl.sendText(msg.to,"done")
	elif msg.text in ["è‡ªå‹•å�‚åŠ :ã‚ªãƒ³","Join on","Auto join:on","è‡ªå‹•å�ƒåŠï¼šé–‹"]:
	if wait["autoJoin"] ==True:
	if wait["lang"] =="JP":
	...
	if wait["lang"] =="JP":
	                        ...
	                        cl.sendText(msg.to,"done")
	elif msg.text in ["è‡ªå‹•å�‚åŠ :ã‚ªãƒ•","Join off","Auto join:off","è‡ªå‹•å�ƒåŠï¼šé—œ"]:
	...
	try:
	                        num =int(strnum)
	                        group = cl.getGroup(msg.to)
	for var inrange(0,num):
	                            name ="".join([random.choice(source_str) for x inxrange(10)])
	                            time.sleep(0.01)
	                            group.name = name
	...
	exceptEOFError:
	raiseException("It might be wrong revision\n"+str(cl.Poll.rev))
